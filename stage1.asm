...
start:
	...
	get_environment_variable:
		...
		jz      assemble_ok
		mov     cell[current_line],_eax
		cmp     [error],errors.undefined_symbol
		jne     error_confirmed
		mov     eax,[error_info]
		or      eax,eax
		jz      error_confirmed
		...
		mov     edx,errors.undefined_symbol
		test    byte [eax+8],1
		jz      addressing_space_unavailable
		mov     edx,errors.symbol_out_of_scope
		mov     cx,[eax+16]
		cmp     cx,[current_pass]
...
errors: ret
	...
	.undefined_symbol:
		...
		push	message
		cmp	[error_info],0
		je	errors.with_source
		mov	esi,[error_info]
		mov	esi,[esi+24]
		or	esi,esi
		jz	errors.with_source
		mov	byte [edi-1],20h
	...
	.symbol_out_of_scope:
		...
		mov	byte [edi-1],20h
		call	write_quoted_symbol_name
		jmp .finish_symbol_out_of_scope_message
		ret
