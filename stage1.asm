...
macro read.then_emit_character
{
	read
	mov dl, al
...
start:
		sub rsp, 128
		push rax rbx rcx rdx rbp rsp rsi rdi r8 r9 r10 r11 r12 r13 r14 r15
		mov r15, 0xFFFFFFF5 ; STD_OUTPUT_HANDLE
		mov esi, _logo
		call strands.emit
	...
	find_param:
		...
		add	rsp,40
		do_exit:
		pop r15 r14 r13 r12 r11 r10 r9 r8 rdi rsi rsp rbp rdx rcx rbx rax
		add rsp, 128
...
hashes: ret
		...
		.create_symbol_entry:
			...
			mov [ edx ], eax
			mov [ ebx ], edx
			jmp hashes.create_symbol_entrance
			ret
		
		.create_symbol_entrance:
			pop	_esi _edi
			mov [ edx+4 ], esi
			ret
...
lex: ret
		...
		.created_definition_backslash_character:
			...
			@@:
				cmp	al, 2Ah
				jne	lex.create_multiline_comment
				lods byte [ esi ]
				cmp	al, '/'
				jne	lex.create_multiline_comment
				jmp	lines.edit_line_data
		...
		@@:
			cmp	al, 2Ah
			jne	lex.signal_skip_multiline_comment
			lods byte [ esi ]
			cmp	al, '/'
			jne	lex.signal_skip_multiline_comment
			jmp lines.append
		...
		convert_separator:
			...
			ret
/*
flat | lines | Line Processor Handling */	
lines: ret
		/*
		flat | lines::create */
		/*
...
/*
DIRECTIVES | All directives must remain below this comment */
/*
