...
hashes: ret
	...
	.read_leaf_symbol:
		...
		lea	ebx,[edx+eax*4]
		dec	ecx
		jnz	.read_leaf_symbol
		mov	edx,[ebx]
		or	edx,edx
		jz	.create_entrance
		shr	ebp,30
		cmp	ebp,11b
		je .signal_entrance_reuse
		cmp	dword [edx+4 ], 0
		jne	.create_entrance
		jmp .read_leaf_entrance
		ret
	
	.read_leaf_entrance:
		mov	edi,[edx]
		or	edi,edi
		jz	.signal_entrance_reuse
		cmp	dword [edi+4 ], 0
		jne	.signal_entrance_reuse
		mov	edx,edi
		jmp	.read_leaf_entrance
		ret
	/*
	...
	.signal_entrance_reuse:
		pop	_esi _edi
		mov	[edx+4 ], esi
		ret
/*
flat | symbols | Symbolic Handling */
symbols: ret
	/*
	flat | symbols::create */
	.create_labels:
...
characters: ret
	...
	.signal_emit_ok:
		ret
/*
flat | strands | ASCII Characters Handling */
strands: ret
	/*
	flat | strands::create */
	.create:
...
system: ret
	...
	.signal_read_nothing_error:
		stc
		ret
		
	.signal_read_bytes_mismatch_error:
		stc
		ret
		
	.signal_read_from_offset_error:
		stc
		ret
...
lex: ret
	...
	.create_symbols:
		...
		sub	ebx,ecx
		mov	byte [ ebx-2 ], cl
		jmp .create_separator
		ret
	
	.create_symbol:
		cmp	al,3Bh
		je .signal_skip_comment
		cmp	al, 2Fh
		jne	@f
		lods	byte [ esi]
		cmp	al, 2Ah
	...
	.create_backslashes:
		lods	byte [ esi]
		cmp	al,5Ch
		jne	.create_backslash_symbols
		stos	byte [ edi]
		inc	byte [ ecx]
		jz	errors.name_too_long
		jmp	.create_backslashes
		ret
	...
	.create_backslash_symbols:
		...
		xlat	byte [ ebx]
		or	al,al
		jz	.create_backslash_character
		mov	al,ah
		jmp .create_backslash_symbols
		ret
	...
	.create_constant:
		...
		jmp	.signal_initial_lexing_ok
		ret
		
	.create_label_constant:
		mov	ebx,[edx+8]
		mov	ecx,[edx+12]
		add	ecx,ebx
		jmp .test_broken_label
		ret
		
	.created_label_constant:
		mov	ecx,edi
		sub	ecx,esi
		mov	edi,[edx+12]
		add	edi,ebp
	...
	.create_prepared_constant:
		mov	byte [ esi-2 ], 3Bh
		mov	cl,[esi-1]
		call hashes.create_symbols
		pop	_ebx
		mov	ecx,edi
		dec	ecx
		sub	ecx,ebx
	...
	.read_current_line:
		...
		add	esi,16
		cmp	word [esi ], 3Bh
		jne	.create_current_line
		add	esi,2
		jmp .create_current_line
		ret
	...
	.read_instruction:
		...
		inc	esi
		cmp	al,1Ah
		jne	.signal_not_preprocessor_symbol
		cmp	cl,3
		jb	.signal_not_directive
		push _edi
		mov	edi,preprocessor_directives
		call .read_directive
		pop	_edi
		jc	.signal_not_directive
		mov	byte [ edx-2 ], 3Bh
		jmp	near _eax
		ret
	...
	.read_labels:
		...
		jnc	.create_label_constant
		lea	esi,[esi+ecx+1]
		cmp	byte [ esi ], ':'
		jne	.read_instruction
		inc	esi
	...
	.test_directives:
		...
		cmp	ah,[edi]
		jb	.test_directive
		ja	.read_next_directive
		cmp	cl,al
		jne	.read_next_directive
		repe	cmps byte [ esi ], [edi]
		jb	.test_directive
		je .signal_directive
		jmp .read_next_directive
		ret
	...
	.test_broken_label:
		cmp	ebx,ecx
		je .signal_label_broken
		cmp	byte [ ebx ], 1Ah
		jne	.signal_label_broken
		movzx	eax,byte [ ebx+1]
		lea	ebx,[ebx+2+eax]
		cmp	ebx,ecx
		je .created_label_constant
		cmp	byte [ ebx ], ':'
		jne	.signal_label_broken
		inc	ebx
		cmp	byte [ ebx ], ':'
		jne	.test_broken_label
		inc	ebx
		jmp	.test_broken_label
		ret
	/*
	...
	.signal_skip_comment:
		...
		jz	.created_line
		cmp	al,1Ah
		jne	.signal_skip_comment
		jmp .created_line
		ret
	
	.signal_directive:
		call .read_directive_handler
		jmp .create_directive
		ret
		
	.signal_initial_lexing_ok:
		mov	_esi,cell[current_line]
		add	esi,16
		mov	al,[macro_status]
		test	al,2
	...
	.signal_not_macro:
		...
		lods	byte [ esi]
		cmp	al,3
		jne	.signal_not_symbolic_constant
		mov	ebx, chars
		movzx	eax,byte [ esi]
		xlat	byte [ ebx]
		ror	eax,8
		...
		ror	eax,16
		cmp	eax,'equ'
		je .create_equ_constant
		mov	al,3
		jmp .signal_not_symbolic_constant
		ret
		
	.signal_not_symbolic_constant:
		mov	ch,1
		mov	cl,al
		call symbols.read_symbols
		jc	.signal_not_preprocessor_symbol
		push _edx _esi
		mov	_esi,cell[struc_name]
		...
		.signal_label_broken:
		push .signal_line_preprocessed
		jmp	replace_symbolic_constant
		ret
		
		.signal_not_preprocessor_symbol:
		mov	_esi,cell[current_offset]
		call process_equ_constants
		jmp .signal_line_preprocessed
		ret
		
		.signal_line_preprocessed:
		pop	_esi _ecx
		ret
/*
flat | macros | Auto Instruction Handling */
macros: ret
	/*
	flat | macros::create */
	.create:
	...
	.create_macros:
		...
		lods	byte [ esi]
		or	al,al
		jz	lex.signal_line_preprocessed
		cmp	al,'{'
		je .signal_block
		dec	esi
		jmp .signal_skip_rules
		ret
		
	.create_macro:
		call process_macro_operators
		jmp lex.signal_initial_lexing_ok
		ret
	
	.create_rule_with_default_value:
		or	[skip_default_argument_value ], -1
		call .signal_skip_rule_value
		inc	esi
		jmp	.signal_rule_end
		ret
	/*
	...
	.signal_skip_rules:
		...
		lods	byte [ esi]
		cmp	al,':'
		je .create_rule_with_default_value
		cmp	al,'='
		je .create_rule_with_default_value
		cmp	al,'*'
		jne	.signal_rule_end
	...
	.signal_rule_end:
		...
		jmp .create_rule_with_default_value
		ret
	...
	.signal_skip_block:
		...
		jz	lex.signal_line_preprocessed
		cmp	al,'}'
		jne	.signal_skip_block
		mov	al,[macro_status]
		and [ macro_status ], 0F0h
		test	al,8
		jnz	use_instant_macro
		cmp	byte [ esi ], 0
		je lex.signal_line_preprocessed
		mov	ecx,edi
		sub	ecx,esi
		mov	edx,esi
		lea	esi,[esi+ecx-1]
	...
	.signal_block:
		...
		or	al,al
		jnz	errors.invalid_rules
		jmp	lex.signal_line_preprocessed
		ret
		...
		cmp	byte [ esi ], '\'
		je escaped_symbol
		jmp symbol_before_macro_operators_ok
		ret
		...
		cmp	byte [ esi ], '\'
		je escaped_symbol
		jmp symbol_after_macro_operatorss_ok
		ret
		...
		cmp	byte [ esi ], '`'
		jne	no_concatenation
		jmp concatenate_converted_symbol
		ret
		...
		inc	esi
		dec	ecx
		jmp finish_concatenating_converted_symbol
		ret
		...
		push    cell[current_line]
		pop     cell[error_line]
		mov     [error ], errors.value_out_of_range
		or      [value_undefined ], -1
		ignore_overflow:
		ret
		recoverable_misuse:
		...
		push    cell[current_line]
		pop     cell[error_line]
		mov     [error ], errors.invalid_use_of_symbol
		ignore_misuse:
		ret
		get_word_value:
		mov     [value_size ], 2
		...
		push    cell[current_line]
		pop     cell[error_line]
		mov     [error ], errors.invalid_address
		...
		mov     [error_info ], eax
		jmp     errors.signal_undefined_symbol
		...
		ja      errors.invalid_use_of_symbol
		...
		elf_segment:
		bt      [format_flags ], 0
		jnc     errors.illegal_instruction
		test    [format_flags ], 8
		jnz     elf64_segment
		...
		elf_segment_type:
		cmp     byte [ ebx ], 1
		jne     errors.setting_already_specified
		lods    word [esi]
		mov     ecx,[number_of_sections]
		...
		cmp     [edx ], ah
		je      errors.data_already_defined
		add     edx,20h
		...
		basic_mem_imm_64bit:
		cmp     [size_declared ], 0
		jne     errors.long_immediate_not_encodable
		call    operand_64bit
		call    get_simm32
		cmp     [value_type ], 4
		jae     errors.long_immediate_not_encodable
		...
		push    cell[current_line]
		pop     cell[error_line]
		mov     [error ], errors.operand_size_not_specified
		ignore_unknown_size:
		ret
		...
		jnz     errors.invalid_operand_size
		test    [rex_prefix ], 8
		jz      errors.invalid_operand_size
		jmp     instruction_ready
...
errors: ret
	...
	preprocessor_directives:
		db 6,'define'
		dw lex.create_static-lex.create_directive
		db 7,'include'
		dw include_file-lex.create_directive
		db 3,'irp'
		dw irp_directive-lex.create_directive
		db 4,'irps'
		dw irps_directive-lex.create_directive
		db 4,'irpv'
		dw irpv_directive-lex.create_directive
		db 5,'macro'
		dw macros.create-lex.create_directive
		db 5,'match'
		dw match_directive-lex.create_directive
		db 8,'postpone'
		dw postpone_directive-lex.create_directive
		db 5,'purge'
		dw purge_macro-lex.create_directive
		db 4,'rept'
		dw rept_directive-lex.create_directive
		db 7,'restore'
		dw restore_equ_constant-lex.create_directive
		db 7,'restruc'
		dw purge_struc-lex.create_directive
		db 5,'struc'
		dw define_struc-lex.create_directive
		db 0

		macro_directives:
		db 6,'common'
		dw common_block-lex.create_directive
		db 7,'forward'
		dw forward_block-lex.create_directive
		db 5,'local'
		dw local_symbols-lex.create_directive
		db 7,'reverse'
		dw reverse_block-lex.create_directive
		db 0
