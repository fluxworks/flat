...
/*
Flat Assembler x64 | Start
Copyright (c) 2020-2025, AppBranch; All rights reserved. */
...
find_command_start:
lodsb
equal find_command_start, ' '
equal skip_quoted_name, '"'
skip_name:
lodsb
equal find_param, ' '
or al, al
...
option_param:
lodsb
equal memory_option, 'm', 'M'
equal passes_option, 'p', 'P'
equal definition_option, 'd', 'D'
equal symbolics_option, 's', 'S'
/**/
bad_params:
stc
ret
...
/**/
symbolics_option:
mov [ symbolics_file ], edi
find_symbolics_file_name:
lodsb
cmp al, 20h
jne process_param
jmp find_symbolics_file_name
/**/
param_end:
dec esi
string_param_end:
...
ret
/*
Flat Assembler x64 | Includes
Flat Assembler x64 | System */
CREATE_NEW = 1
CREATE_ALWAYS = 2
OPEN_EXISTING = 3
OPEN_ALWAYS = 4
...
(
lpName: LPCTSTR?, The name of the environment variable.
lpBuffer:= LPTSTR, A buffer that receives the contents of the variable as a null-terminated string.
nSize: LPTSTR, The size of the buffer, including the null-terminating character, in characters.
)-> DWORD */
...
/*
flat | characters | C style terminated ASCII strands */
characters: ret
		/*
		...
		.create_control:
			...
			je lines.signal
			cmp al, 0Dh
			je characters.create_carriage_return
			cmp al, 0Ah
			je characters.create_linefeed
			cmp al, 9
			je lines.create_line
			or al, al
			jnz symbols.create_characters
			jmp lines.signal
			ret
		...
		.create_backslash:
			...
			stos word [ edi ]
			dec esi
			jmp characters.edit_backslashes
			ret
		...
		.edit_backslashes:
			lods byte [ esi ]
			cmp al, 5Ch
			jne symbols.create_backslashed
			stos byte [ edi ]
			inc byte [ ecx ]
			jz name_too_long
			jmp characters.edit_backslashes
			ret
		/*
		flat | characters::append */
		/*
		flat | characters::test */
		.test:
				push _ecx
				sub rsp,40
				mov rcx,qword[con_handle]
		...
		.signal:	ret
/*
flat | strands | C style terminated ASCII strands */
strands: 		ret
		.emit:
				lods byte [ esi ]
				stos byte [ edi ]
		...
		.emitted:
				...
				ret
/*
flat | symbols | C style terminated ASCII strands */
symbols: 
				mov edx, [ symbolics_file ]
				call create
				jc write_failed
				mov edx, [ code_start ]
				mov ecx, [ edx+14h ]
		...
		.create:
				...
				je symbols.create_reference
				mov eax, [ esi-4 ]
				mov ecx, [ esi-8 ]
				sub esi, 8
				sub esi, ecx
				cmp eax, 2
				je symbols.create_references
				cmp eax, 1
				jne symbols.create
				mov edx, [ esi ]
				jmp symbols.create
		...
		.create_prepared_strands:
			cmp edx,ebp
			je symbols.created_prepared_strands
			mov al, [ edx ]
			test al, al
			jz symbols.create_prepared_strand
			cmp al, 80h
			je symbols.create_prepared_strand
			add edx, 0Ch
			cmp al, 0C0h
			jb symbols.create_prepared_strands
			add edx,4
			jmp symbols.create_prepared_strands
			ret
			
		.create_prepared_strand:
			mov esi, edi
			sub esi, ebx
			xchg esi, [ edx+4 ]
			test al, al
			jz symbols.create_prepared_section
			or dword [ edx+4 ], 1 shl 31
			add edx, 0Ch
			jmp symbols.create_prepared_external
			ret
			
		.create_prepared_external:
			mov ecx, [ esi ]
			add esi, 4
			rep movs byte [ edi ], [ esi ]
			mov byte [ edi ], 0
		...
		.create_prepared_section:
			...
			add edx,20h
			test esi, esi
			jz symbols.create_default_section
			cmp [ output_format ], 5
			jne symbols.create_prepared_external
			bt [ format_flags ], 0
			jc symbols.create_prepared_external
			mov esi, [ esi ]
			add esi, [ resource_data]
			jmp symbols.emit_prepared_section_name
			ret
		...
		.created_prepared_strands:
			mov edx, [ tagged_blocks ]
			mov _ebp, cell[ memory_end ]
			sub ebp, [ labels_list ]
			add ebp,edx
			jmp symbols.create_labels
			ret
			
		.create_labels:
			cmp edx,ebp
			je symbols.created_labels
			mov eax, [ edx+24 ]
			test eax, eax
			jz symbols.created_label
			cmp _eax, cell[ memory_start ]
			jb symbols.create_external_label
			cmp eax, [ source_start ]
			ja symbols.create_external_label
			sub _eax, cell[ memory_start ]
			dec eax
			mov [ edx+24 ], eax
			jmp symbols.created_label
			ret
		...
		.created_label:
			mov eax, [ edx+28 ]
			test eax, eax
			jz symbols.created_line
			sub _eax, cell[ memory_start ]
			mov [ edx+28 ], eax
			jmp symbols.created_line
			ret
			
		.created_line:
			test byte [ edx+9 ], 4
			jz symbols.create_base
			xor eax, eax
			mov [ edx ], eax
			mov [ edx+4 ], eax
			jmp symbols.created_base
			ret
			
		.create_base:
			mov eax, [ edx+20 ]
			test eax, eax
			jz symbols.created_base
			cmp eax, [ symbolics_stream ]
			mov eax, [ eax+4 ]
			jae symbols.created_base
			xor eax, eax
			jmp symbols.created_base
			ret
		...
		.created_base:
			...
			jmp symbols.create_flag
			ret
			
		.create_flag:
			cmp ax, [ edx+18 ]
			je symbols.created_flag
			and byte [ edx+8 ], not 8
			jmp symbols.created_flag
			ret
		...
		.created_labels:
			...
			mov esi, [ labels_list ]
			mov ebp,edi
			jmp symbols.create_lines
			ret
			
		.create_lines:
			cmp esi, [ tagged_blocks ]
			je symbols.created_lines
			mov eax, [ esi-4 ]
			mov ecx, [ esi-8 ]
			sub esi, 8
			sub esi, ecx
			cmp eax, 1
			je symbols.read_line
			cmp eax, 2
			jne symbols.create_lines
		...
		.create_offset:
			...
			mov eax, [ ebx+14h ]
			test eax, eax
			jz symbols.created_line_base
			cmp eax, [ symbolics_stream ]
			mov eax, [ eax+4 ]
			jae symbols.created_line_base
			xor eax, eax
			jmp symbols.created_line_base
			ret
		...
		.create_characters:
			cmp al, 3Bh
			je lines.signal_skip_comment
			cmp al, 2Fh
			jne @f
			lods byte [ esi ]
			cmp al, 2Ah
			je lines.create_comment
			dec esi
			dec esi
			lods byte [ esi ]
			@@:
			cmp al, 5Ch
			je characters.create_backslash
			stos byte [ edi ]
			jmp lines.create_line
			ret
		...
		.create_backslashed:
			...
			je extra_characters_on_line
			mov ah,al
			mov ebx, characters_buffer
			mov ebx, characters_buffer
			xlat byte [ ebx ]
			or al, al
			jz characters.create_backslashed_symbol
			mov al, ah
			jmp symbols.edit_backslashed
			ret
		/*
		flat | symbols::read */
		.read:
		...
		.read_symbol:
			...
			ja symbols.read_up
			jb symbols.read_down
			mov ax, [ edi ]
			cmp al, 18h
			jb symbols.read_symbol_ok
			cmp al, 1Fh
			je symbols.read_decorator
			cmp [ formatter_symbolics_allowed],0
			je symbols.signal
			jmp symbols.read_symbol_ok
			ret
		...
		.read_decorator:
			cmp [ decorator_symbolics_allowed],0
			jne symbols.read_symbol_ok
			jmp symbols.signal
			ret
		...
		.read_line:
			...
			add eax, [ headers_size ]
			test byte [ ebx+0Ah ], 1
			jz symbols.create_offset
			xor eax, eax
			jmp symbols.create_offset
			ret
		...
		.read_offsets:
			...
			jb symbols
			test byte [ edx+1Ah ], 1
			jnz symbols.read_offsets
			cmp eax, [ edx ]
			jb symbols.emit_offset
			mov eax, [ edx ]
			jmp symbols.read_offsets
			ret
		...
		.emit:
			...
			cmp edi, [ tagged_blocks ]
			jae out_of_memory
			mov edx, [ symbolics_stream ]
			mov _ebp, cell[ free_additional_memory ]
			and [ number_of_sections ], 0
			cmp [ output_format ], 4
			je symbols.create_prepared_strands
			cmp [ output_format ], 5
			jne symbols.created_prepared_strands
			bt [ format_flags ], 0
			jc symbols.created_prepared_strands
			jmp symbols.create_prepared_strands
			ret
		...
		.emit_prepared_section_name:
			...
			stos byte [ edi ]
			test al, al
			jnz symbols.emit_prepared_section_name
			jmp symbols.create_prepared_strands
			ret
		/*
		flat | symbols::signal */
		...
		.signal_unscoped_errors:
			mov edi,message
			mov esi, _symbol_out_of_scope_1
			call strands.emit
			cmp [ error_info],0
			je symbols.signal_unscoped_error
			mov esi, [ error_info]
			mov esi, [ esi+24 ]
			or esi, esi
			jz symbols.signal_unscoped_error
			mov byte [ edi-1 ], 20h
			call symbols.emit_quoted_name
			jmp symbols.signal_unscoped_error
			ret
			
		.signal_unscoped_error:
			mov byte [ edi-1 ], 20h
...
lex: ret
		...
		.create_characters:
			...
			mov esi, characters_buffer+'a'
			mov edi,characters_buffer+'A'
			mov ecx, 26
			rep movsb
			mov edi,characters_buffer
		...
		.create_separators:
			xchg al, ah
			cmp al, 20h
			jb characters.create_control
			je lines.create_line
			jmp symbols.create_characters
			ret
		...
		.create_case:
			...
			xlat byte [ ebx ]
			stos byte [ edi ]
			loop lex.create_case
			jmp lex.created_case
			ret
		...
		.edit:
			...
			xlat byte [ ebx ]
			or al, al
			jz lex.create_separator
			cmp ah,27h
			je lex.create_strand
			cmp ah,22h
			je lex.create_strand
			mov byte [ edi ], 1Ah
			scas word [ edi ]
		...
		.edit_strand:
			...
			stos byte [ edi ]
			cmp al, 0Ah
			je lex.signal_no_end_quote
			cmp al, 0Dh
			je lex.signal_no_end_quote
			or al, al
			jz lex.signal_no_end_quote
			cmp al, 1Ah
			je lex.signal_no_end_quote
			cmp al, ah
			jne lex.edit_strand
...
lines: ret
		...
		.signal_skip_comment:
			...
			jz lines.signal
			cmp al, 1Ah
			jne lines.signal_skip_comment
/*
flat | directs | Processes The Subcommand Before Assembly */
directs:
		push _edi
		mov edx,esi
		mov ebp, ecx
	...
	.test:
		...
		flat | directs::signal */
		.signal:
			call directives.create
			jmp directives
			ret
/*
flat | lex | Processes The Command Before Assembly */
directives:
			lea esi, [ edx+ebp]
			movzx ecx, word [ ebx ]
			add eax, ecx
		...
		lods byte [ esi ]
		or al, al
		jz lines.created_line
		cmp al, '{'
		jne unexpected_characters
		jmp macros.read_block_ok
		rept_directive:
		mov [ base_code ], 0
		jmp macros.create_iteration
		ret
		...
		call rules.signal_skip_rule_value
		...
		mov [ ebx+12 ], eax
		mov [ ebx+8 ], ecx
		jmp collect_next_variable_value
		ret
...
/*
flat | blocks | Closures For Directives And Macros */
blocks: ret
        /*
        flat | blocks::create */
		.create:
...
macros: ret
	...
	flat | macros::create */
		...
		.create_macro:
            ...
            je macros.read_block_ok
            dec esi
            jmp rules.signal_skip_rules
            ret
			
		.create_symbols:
			push _ebx _ebp
			call macros.read_symbol_leaf
			jc macros.edit_symbols_tree
			mov eax, [ ebx ]
		...
		.create_escaped_symbol:
			dec byte [ edi-1 ]
			dec ecx
			inc esi
			cmp ecx, 1
			rep movs byte [ edi ], [ esi ]
			jne macros.signal_operators
			mov al, [ esi-1 ]
			mov ecx, ebx
			mov ebx, characters_buffer
			xlat byte [ ebx ]
		...
		.create_operators:
			xor dl,dl
			mov ebp,edi
			jmp macros.read_operators
			ret
			
		.create_operator_symbol:
			add esi, ecx
			jmp macros.read_operators
			ret
			
		.create_operator_symbol_strand:
			mov dl,22h
			mov ebx, esi
			lods dword [ esi ]
			add esi, eax
		...
		.created_operator_symbol_strand:
			mov dl,22h
			mov ebx, edi
			lods dword [ esi ]
			stos dword [ edi ]
		...
		.read:
			...
			mov al, [ macro_status ]
			test al, 2
			jnz macros.signal_skip_block
			test al, 1
			jnz macros.read_block
			jmp macros.read_instruction
			ret
		...
		.read_symbols:
            push _ecx
            call macros.read_symbol_leaf
            jc macros.signal_symbol_not_found
            mov edx, [ ebx ]
            mov ebx, esi
            jmp macros.test_symbol
            ret
		...
		.read_instruction:
			...
			jne not_preprocessor_symbol
			cmp cl,3
			jb macros.test_symbols
			push _edi
			mov edi,preprocessor_directives
			call directs
			pop _edi
			jc macros.test_symbols
			mov byte [ edx-2 ], 3Bh
			jmp near _eax
			ret
		/*
		NOTE: Need to add a jump table for jecxz instruction below */
		.creates_operator_symbols: 
			jmp macros.create_operator_symbol
			ret
		...
		.read_operators:
			...
			lods byte [ esi ]
			cmp al, '`'
			je macros.edit_symbol
			cmp al, '#'
			je macros.append
			cmp al, 1Ah
			je macros.read_operator_symbol
			cmp al, 3Bh
			je macros.test_operators
			cmp al, 22h
			je macros.create_operator_symbol_strand
			xor dl,dl
			or al, al
			jnz macros.read_operators
			mov edi,esi
			ret
			
		.read_operator_symbol:
			mov dl,1Ah
			mov ebx, esi
			lods byte [ esi ]
			movzx ecx, al
			jecxz macros.creates_operator_symbols
			mov edi,esi
			cmp byte [ esi ], '\'
		...
		.edit_symbol:
			...
			mov al, [ esi ]
			cmp al, 1Ah
			jne macros.edit_symbol_character
			lods word [ esi ]
			movzx ecx, ah
			lea ebx, [ edi+3 ]
			jecxz macros.edit_quoted_strand
			cmp byte [ esi ], '\'
			jne macros.edit_quoted_strand
			inc esi
			dec ecx
			dec ebx
			jmp macros.edit_quoted_strand
			ret

		.edit_symbol_character:
			cmp al, 22h
			je macros.signal_operators
			cmp al, '`'
			je macros.edit_symbols
			lea ebx, [ edi+5 ]
			xor ecx, ecx
			or al, al
			jz macros.edit_quoted_strand
			cmp al, '#'
			je macros.edit_quoted_strand
			inc ecx
			jmp macros.edit_quoted_strand
			ret
		...
		.append:
			cmp dl,1Ah
			je macros.append_symbols
			cmp dl,22h
			je macros.append_strands
			jmp macros.signal_no_append
			ret
		
		.append_symbols:
			cmp byte [ esi ], 1Ah
			jne macros.signal_no_append
			inc esi
			lods byte [ esi ]
			movzx ecx, al
			jecxz macros.append_symbol
			cmp byte [ esi ], '\'
			je macros.append_escaped_symbol
			jmp macros.append_symbol
			ret
		...
		.appended_symbol:
			...
			mov al, [ esi ]
			cmp al, '`'
			je macros.appended_symbol
			cmp al, 22h
			je macros.appended_strand
			cmp al, 1Ah
			jne macros.appended_symbol_characters
			inc esi
			lods byte [ esi ]
			movzx ecx, al
			jecxz macros.appended_symbols
			cmp byte [ esi ], '\'
			jne macros.appended_symbols
			inc esi
			dec ecx
			jmp macros.appended_symbols
			ret
			
		.appended_symbol_characters:
			or al, al
			jz macros.signal_operators
			cmp al, '#'
			je macros.signal_operators
			inc dword [ ebx ]
			movs byte [ edi ], [ esi ]
		...
		.append_escaped_symbol:
			inc esi
			dec ecx
			jz macros.append_symbol
			movzx eax, byte [ esi ]
			cmp byte [ characters_buffer+eax ], 0
			jne macros.append_symbol
			sub esi, 3
			jmp macros.signal_no_append
			ret
        ...
        .test_symbol:
            or edx, edx
            jz macros.signal_symbol_not_found
            mov _ecx, [ _esp ]
            mov edi, [ edx+4 ]
            repe cmps byte [ esi ], [ edi ]
            je macros.signal_symbol_found
            mov esi, ebx
            mov edx, [ edx ]
            jmp macros.test_symbol
            ret
		...
		.test_symbols_tree:
            mov edx, [ ebx ]
            or edx, edx
            jz macros.signal_no_such_symbol
            xor eax, eax
            shr ebp,1
            adc eax, 0
            lea ebx, [ edx+eax*4 ]
            or ebp,ebp
            jnz macros.test_symbols_tree
            add ebx, 8
            clc
            ret
		...
		.signal_operators:
			...
			stos byte [ edi ]
			cmp al, 1Ah
			je macros.signal_operator_symbol
			cmp al, 3Bh
			je macros.test_operators
			cmp al, 22h
			je macros.created_operator_symbol_strand
			xor dl,dl
			or al, al
			jnz macros.signal_operators
			ret
		...
		.signal_operator_symbol:
			mov dl,1Ah
			mov ebx, edi
			lods byte [ esi ]
			stos byte [ edi ]
			movzx ecx, al
			jecxz macros.signal_operator_symbol_ok
			cmp byte [ esi ], '\'
			je macros.create_escaped_symbol
			jmp macros.signal_operator_symbol_ok
			ret
			
		.signal_operator_symbol_ok:
			rep movs byte [ edi ], [ esi ]
			jmp macros.signal_operators
			ret
		...
		.signal_no_such_symbol:
            stc
            ret

/*
flat | rules | Parameters of macros */
rules: ret
		/*
		flat | rules::create */
		.create:
			...
			jz invalid_macro_arguments
			cmp al, 1Ah
			je rules.create_symbols
			cmp al, 22h
			je rules.create_strands
			cmp al, '>'
			je rules.created
			cmp al, '<'
			jne rules.create
		...
		.create_simple_rule:
			...
			je rules.signal
			cmp al, 22h
			je rules.create_strand
			cmp al, 1Ah
			je rules.create_symbol
			cmp [ skip_default_argument_value ], 0
			je rules.create_simple_rule
			cmp al, '{'
			je rules.signal
			cmp al, '&'
			je rules.signal
			or ebp,ebp
			jz rules.create_simple_rule
			cmp al, ']'
			je rules.signal
			jmp rules.create_symbol
			ret

		.create_rule_with_default_value:
			or [ skip_default_argument_value ], -1
			call rules.signal_skip_rule_value
			inc esi
			jmp rules.signal_rule_end
			ret
		...
		.read:
			...
			inc esi
			cmp al, '['
			jne rules.create_rules
			mov ebp,esi
			inc esi
			inc [ counter_limit ]
			jmp rules.create_rules
			ret
		...
		.signal_skip_rule:
			...
			lods byte [ esi ]
			cmp al, ':'
			je rules.create_rule_with_default_value
			cmp al, '='
			je rules.create_rule_with_default_value
			cmp al, '*'
			jne rules.signal_rule_end
			lods byte [ esi ]
			jmp rules.signal_rule_end
			ret
		...
		.signal_skip_rule_value:
			cmp byte [ esi ], '<'
			jne rules.create_simple_rule
			mov ecx, 1
			inc esi
			jmp rules.create
			ret
			
		.signal_rule_value_length:
			mov [ edx+8 ], ecx
			ret

		.signal_finish_rules:
			lods byte [ esi ]
			jmp rules.signal_rules_end
			ret
		...
		.signal_finish_rule:
			...
			sub ecx, eax
			cmp byte [ eax ], '<'
			jne rules.signal_rule_value_length
			inc dword [ edx+12 ]
			sub ecx, 2
			or ecx, 80000000h
			jmp rules.signal_rule_value_length
			ret
		...
		.signal_rule_end:
			cmp al, ','
			je rules.signal_skip_rules
			cmp al, '&'
			je rules.signal_finish_rules
			cmp al, ']'
			jne rules.signal_rules_end
			not ebp
			jmp rules.signal_finish_rules
			ret
/*
...
add esi, 16
parse_line:
mov [ formatter_symbolics_allowed],0
mov [ decorator_symbolics_allowed],0
cmp byte [ esi ], 1Ah
...
or [ formatter_symbolics_allowed ], -1
parse_argument:
lea eax, [ edi+100h ]
cmp eax, [ labels_list ]
jae out_of_memory
...
ret
/*
flat | expressions | Expressions Handling */
expressions:
				mov al, '('
				stos byte [ edi ]
				call convert_expression
				...
				ret
			/*
			flat | expressions::create */
			/*
			flat | expressions::read */
			/*
			flat | expressions::edit */
			/*
			flat | expressions::append */
			/*
			flat | expressions::test */
			/*
			flat | expressions::emit */
			/*
			flat | expressions::signal */
not_string:
cmp byte [ esi ], '('
jne expression
mov _eax,_esp
...
forced_expression:
xor al, al
xchg al, [ formatter_symbolics_allowed]
push _eax
call expressions
...
pop _eax
mov [ formatter_symbolics_allowed ], al
jmp argument_parsed
ret
...
open_decorator:
inc [ decorator_symbolics_allowed]
jmp separator
ret

close_decorator:
dec [ decorator_symbolics_allowed]
jmp separator
ret
...
/*
Flat Assembler x64 | Expressions */
convert_expression:
push _ebp
call get_fp_value
jnc fp_expression
...
jz assemble_ok
mov cell[ current_line ], _eax
cmp [ error], symbols.signal_undefined_symbol_error
jne error_confirmed
mov eax, [ error_info]
or eax, eax
jz error_confirmed
...
mov edx, symbols.signal_undefined_symbol_error
test byte [ eax+8 ], 1
jz addressing_space_unavailable
mov edx, symbols.signal_unscoped_errors
mov cx, [ eax+16 ]
cmp cx, [ current_pass]
...
/**/
/*
Flat Assembler x64 | Interpreter */
...
mov eax, [ number_of_sections ]
mov [ ebx+2 ], ax
mov esi, [ symbolics_stream ]
xor eax, eax
xor ecx, ecx
enumerate_symbols:
cmp _esi, cell[ free_additional_memory ]
je symbolics_enumerated
mov dl, [ esi ]
or dl,dl
...
xor eax, eax
shr ecx, 1
jnc zero_symbolics_table
stos word [ edi ]
zero_symbolics_table:
rep stos dword [ edi ]
mov edx, edi
stos dword [ edi ]
mov esi, [ symbolics_stream ]
make_symbolics_table:
cmp _esi, cell[ free_additional_memory ]
je symbolics_table_ok
mov al, [ esi ]
cmp al, 0C0h
...
add esi, 0Ch
jmp make_symbolics_table
ret
/**/
add_section_symbol:
call symbols.create_name
...
mov [ error_info],eax
jmp symbols.signal_undefined_symbol_error
/**/
...
db 6,'define'
dw lex.create_constant-directives
db 7,'include'
...
db 5,'macro'
dw macros.create_macros-directives
db 5,'match'
dw match_directive-directives
db 8,'postpone'
dw postpone_directive-directives
db 5,'purge'
...
macro_directives:
db 6,'common'
dw blocks.create_common-directives
db 7,'forward'
dw blocks.create_forward-directives
db 5,'local'
dw local_symbols-directives
db 7,'reverse'
dw blocks.create_reverse-directives
db 0
...
symbolics:
	dw symbolics_1-symbolics,(symbolics_2-symbolics_1)/(1+2)
	dw symbolics_2-symbolics,(symbolics_3-symbolics_2)/(2+2)
	dw symbolics_3-symbolics,(symbolics_4-symbolics_3)/(3+2)
	dw symbolics_4-symbolics,(symbolics_5-symbolics_4)/(4+2)
	dw symbolics_5-symbolics,(symbolics_6-symbolics_5)/(5+2)
	dw symbolics_6-symbolics,(symbolics_7-symbolics_6)/(6+2)
	dw symbolics_7-symbolics,(symbolics_8-symbolics_7)/(7+2)
	dw symbolics_8-symbolics,(symbolics_9-symbolics_8)/(8+2)
	dw symbolics_9-symbolics,(symbolics_10-symbolics_9)/(9+2)
	dw symbolics_10-symbolics,(symbolics_11-symbolics_10)/(10+2)
	dw symbolics_11-symbolics,(symbolics_end-symbolics_11)/(11+2)
...
_symbolics_marked db ' Symbols have been marked.', 0Dh,0Ah,0
_processing_predefinitions db ' Processing predefinitions...', 0Dh,0Ah,0

align 4
/*
Flat Assembler x64 | Interface Variables */
...
error dq ?
error_line dq ?
free_additional_memory dq ?
macro_block dq ?
macro_block_line dq ?
macro_block_line_number dq ?
macro_line dq ?
macro_symbols dq ?
memory_end dq ?
memory_start dq ?
...
/*
Flat Assembler x64 | Internal Variables */
...
prefix_flags db ?
formatter_symbolics_allowed db ?
decorator_symbolics_allowed db ?
free_address_range db ?

characters_buffer rb 100h
converted rb 100h
message rb 200h
...
stack 10000h
/*
Flat Assembler x64 | Imports */
section '.idata' import data readable writeable
dd 0,0,0, rva kernel32_filename, rva kernel32
dd 0,0,0,0,0
/*
Kernel32 Offsets
...
SetThreadpoolWait
SetThreadpoolWaitEx documentation
...
kernel32_filename db 'KERNEL32.DLL', 0
_ExitProcess dw 0
db 'ExitProcess', 0
_CreateFileA dw 0
db 'CreateFileA', 0
...
/*
Flat Assembler x64 | Relocations */
