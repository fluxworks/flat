...
strands: 		ret
		/*
		strand | emit | Load From [ESI] To Store In [ EDI ], until Terminator is found. | copy_asciiz: */
		.emit:
				lods byte [ esi ]
				stos byte [ edi ]
				test al, al
...
symbols: ret
			...
			.create_references:
				...
				cmp eax, 1
				jne symbols.create_references
				mov edx, [ esi ]
				jmp symbols.create_references
				ret
			/*
			dump_reference:*/
			.create_reference:
			...
			.create_name:
				...
				ret
			/*
			::symbols | emit | Emit Symbol into file | dump_symbols: */
			.emit:
				mov edi, [ code_start ]
...
lex: ret
		...
		.create_symbols:
			...
			call directives.read_directive
		...
		.create_symbol_character:
			...
			@@:
			cmp al, 5Ch
			je lex.create_backslash_character
			stos byte [ edi ]
			jmp lex.create_line
			ret
		...
		.create_separators:
			xchg al, ah
			cmp al, 20h
			jb lex.create_control_character
			je lex.create_line
			jmp lex.create_symbol_character
			ret
		...
		.create_backslash_symbol:
			...
			xlat byte [ ebx ]
			or al, al
			jz lex.create_backslash_characters
			mov al, ah
			jmp lex.read_backslash_symbol
			ret
			
		.create_backslash_characters:
			mov al, ah
			stos byte [ edi ]
			inc byte [ ecx ]
			jmp lex.edit
		...
		.create_control_character:
			...
			je line_end
			cmp al, 0Dh
			je lex.create_carriage_character
			cmp al, 0Ah
			je lex.create_linefeed_character
			cmp al, 9
			je lex.create_line
			or al, al
			jnz lex.create_symbol_character
			jmp line_end
			ret
		...
		.edit_backslash:
			...
			xlat byte [ ebx ]
			or al, al
			jz lex.edit_backslashed_symbol_character
			mov al, ah
			jmp lex.edit_backslashed_symbol
			ret
		...
		.edit_backslashed_symbol_character:
			mov al, ah
			stos byte [ edi ]
			inc byte [ ecx ]
			jmp lex.create_line
...
macros: ret
	...
	flat | macros::create */
		...
		.finish_macro_argument:
			...
			sub ecx, eax
			cmp byte [ eax ], '<'
			jne macros.signal_rule_value_length
			inc dword [ edx+12 ]
			sub ecx, 2
			or ecx, 80000000h
			...
			not ebp
			jmp macro_arguments_finisher
			ret

			macro_arguments_finisher:
			lods byte [ esi ]
			jmp end_macro_arguments
			ret
...
ret
/*
flat | directives | Processes Supercommand During Lexing */
directives: ret
		/*
		...
		.read_directives:
			...
			movzx eax, byte [ edi ]
			or al, al
			jz directives.signal_no_directive
			mov ecx, ebp
			inc edi
			mov ebx, edi
			add ebx, eax
			mov ah, [ esi ]
			cmp ah, [ edi ]
			jb directives.signal_no_directive
			ja directives.read_next_directive
			cmp cl,al
			jne directives.read_next_directive
			repe cmps byte [ esi ], [ edi ]
			jb directives.signal_no_directive
			je directives.signal
			jmp directives.read_next_directive
			ret
		...
		.read_directive:
			...
			call lower_case
			pop _edi
			jmp directives.read_directives
			ret
...
jz assemble_ok
mov cell[ current_line ], _eax
cmp [ error], symbols.undefined_symbol_error
jne error_confirmed
mov eax, [ error_info]
or eax, eax
jz error_confirmed
...
mov edx, symbols.undefined_symbol_error
test byte [ eax+8 ], 1
jz addressing_space_unavailable
mov edx, symbols.unscoped_symbol_errors
mov cx, [ eax+16 ]
cmp cx, [ current_pass]
...
mov [ error_info],eax
jmp symbols.undefined_symbol_error
/**/
...
db 6,'define'
dw lex.create_constant-directives.read
db 7,'include'
dw include_file-directives.read
db 3,'irp'
dw irp_directive-directives.read
db 4,'irps'
dw irps_directive-directives.read
db 4,'irpv'
dw irpv_directive-directives.read
db 5,'macro'
dw define_macro-directives.read
db 5,'match'
dw match_directive-directives.read
db 8,'postpone'
dw postpone_directive-directives.read
db 5,'purge'
dw purge_macro-directives.read
db 4,'rept'
dw rept_directive-directives.read
db 7,'restore'
dw restore_equ_constant-directives.read
db 7,'restruc'
dw purge_struc-directives.read
db 5,'struc'
dw define_struc-directives.read
db 0

macro_directives:
db 6,'common'
dw common_block-directives.read
db 7,'forward'
dw forward_block-directives.read
db 5,'local'
dw local_symbols-directives.read
db 7,'reverse'
dw reverse_block-directives.read
db 0
